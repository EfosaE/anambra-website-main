...
type SpinnerProps = {
  ...
  center?: boolean;       // Optional: center the spinner on screen
};

export default function Spinner({ size = 32, color = '#da9617', center = false }: SpinnerProps) {
  const dimension = typeof size === 'number' ? `${size}px` : size;

  return (
    <div
      role="status"
      className={center ? 'flex items-center justify-center h-screen' : ''}
    >
      <svg
        aria-hidden="true"
